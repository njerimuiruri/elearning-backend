...
export class CertificateService {
  ...
  async getCertificateFromEnrollment(enrollmentId: string) {
    ...
    if (!enrollment.isCompleted || !enrollment.certificateEarned) {
      throw new NotFoundException(
        'Certificate not available - course not completed or certificate not earned',
      );
    }
  ...
  private async buildCertificateData(enrollment: any) {
    ...
    const certificateData = {
      ...
      scoreAchieved:
        enrollment.finalAssessmentScore || enrollment.totalScore || 0,
      instructorName: instructor
        ? `${instructor.firstName || ''} ${instructor.lastName || ''}`.trim()
        : 'Instructor',
      issuedDate:
        enrollment.certificateIssuedAt || enrollment.completedAt || new Date(),
  ...
  async createPDFBuffer(certificate: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      // Score badge
      const score = certificate.scoreAchieved || certificate.score;
      if (score) {
      ...
      doc
        ...
        .fillColor('#9ca3af')
        .text(
          `Certificate ID: ${certificate.certificateNumber || certificate._id}`,
          0,
          pageHeight - 70,
          {
            align: 'center',
      ...
      doc.fontSize(16).text('✓', sealX - 8, sealY + 8);
