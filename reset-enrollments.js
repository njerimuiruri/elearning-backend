...
async function main() {
  ...
  const existing = await enrollCol
    .find({})
    .project({ _id: 1, studentId: 1, moduleId: 1, isCompleted: 1 })
    .toArray();
  console.log(`\nFound ${existing.length} enrollment(s):`);
  existing.forEach((e) => {
    console.log(
      `  - enrollmentId: ${e._id}  student: ${e.studentId}  module: ${e.moduleId}  completed: ${e.isCompleted}`,
    );
  });
  ...
  const moduleIds = [
    ...new Set(existing.map((e) => e.moduleId?.toString()).filter(Boolean)),
  ];
